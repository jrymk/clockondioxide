...
#include <GxEPD2_3C.h>
#include <GxEPD2_7C.h>
#include <fonts/Aero_Matics_Regular/Aero_Matics_Regular8pt7b.h>
...
void setup()
{
...

  {
    ...
    if (!configFile) {
      ...
      while (configFile.available()) {
        sscanf(configFile.readStringUntil('\n').c_str(), "%s = %s\n", key, value);
        config.insert({std::string(key), std::string(value)});
      }
    }
  }

  Serial.printf("SSID: %s\nPassword: %s\n", config.find("wifi.ssid")->second.c_str(), config.find("wifi.password")->second.c_str());

  {
    display.setFont(&Aero_Matics_Regular8pt7b);
    display.fillScreen(GxEPD_WHITE);
    int16_t tbx, tby;
    uint16_t tbw, tbh;
  ...
  WiFi.mode(WIFI_STA);
  WiFi.begin(config.find("wifi.ssid")->second.c_str(), config.find("wifi.password")->second.c_str());
